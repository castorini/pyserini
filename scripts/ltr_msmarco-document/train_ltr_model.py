...
def train_data_loader(task='rank', neg_sample=10, random_seed=12345):
    ...
    if task == 'rank':
        ...
        with open("./collections/msmarco-doc/orcas-doctrain-qrels.tsv") as f:
            for line in f:
        ...
        with open("./collections/msmarco-doc/run.msmarco-doc.train.1000.bm25tuned.txt") as f:
            for line in tqdm(f):
        ...
        sampled_train.to_pickle(f'./collections/msmarco-ltr-document/train_{task}_sampled_with_{neg_sample}_{random_seed}.pickle')
    return sampled_train


def dev_data_loader(task='anserini'):
    if os.path.exists(f'./collections/msmarco-ltr-document/dev_{task}.pickle'):
        dev = pd.read_pickle(f'./collections/msmarco-ltr-document/dev_{task}.pickle')
        print(dev.shape)
        print(dev.index.get_level_values('qid').drop_duplicates().shape)
        print(dev.groupby('qid').count().mean())
        print(dev.head(10))
        print(dev.info())
        dev_qrel = pd.read_pickle(f'./collections/msmarco-ltr-document/dev_qrel.pickle')
        return dev, dev_qrel
    else:
        if task == 'anserini':
            dev = pd.read_csv('./collections/msmarco-doc/run.msmarco-pass-doc.bm25.txt', sep="\t",
                              names=['qid', 'pid', 'rank'], dtype=str)
        dev_qrel = pd.read_csv('./tools/topics-and-qrels/qrels.msmarco-doc.dev.txt', sep="\t",
                               names=["qid", "q0", "pid", "rel"], usecols=['qid', 'pid', 'rel'], dtype=str)
        ...
        dev.to_pickle(f'./collections/msmarco-ltr-document/dev_{task}.pickle')
        dev_qrel.to_pickle(f'./collections/msmarco-ltr-documents/dev_qrel.pickle')
        return dev, dev_qrel


def query_loader():
    queries = {}
    with open('./collections/msmarco-ltr-document/queries.dev.small.json') as f:
        for line in f:
            ...
            queries[qid] = query
    with open('./collections/msmarco-ltr-document/queries.train.json') as f:
        for line in f:
            query = json.loads(line)
            qid = query.pop('id')
            query['analyzed'] = query['analyzed'].split(" ")
...
if __name__ == '__main__':
    ...
    start = time.time()
    fe.add(IbmModel1("collections/msmarco-ltr-passage/ibm_model/title_unlemm","title","title_unlemm","text_unlemm"))
    end = time.time()
    print('IBM model Load takes %.2f seconds'%(end-start))
    start = end
    fe.add(IbmModel1("collections/msmarco-ltr-passage/ibm_model/url_unlemm","url","url_unlemm","text_unlemm"))
    end = time.time()
    print('IBM model Load takes %.2f seconds'%(end-start))
    start = end
