"""
Convert T5X Retrieval checkpoint to PyTorch
This code was modified from
https://github.com/huggingface/transformers/blob/main/src/transformers/models/t5/convert_t5x_checkpoint_to_pytorch.py
...
- Pretrained T5X_Retrieval checkpoints are at https://console.cloud.google.com/storage/browser/t5-data/pretrained_models/t5x/retrieval/. Example:
    gsutil -m cp -r : gs://t5-data/pretrained_models/t5x/retrieval/gtr_base $HOME/
- Create a corresponding config.json for the downloaded checkpoint. Often one already exists,
    e.g. here we can use https://huggingface.co/google/t5-v1_1-base/blob/main/config.json

- Convert:
    ```
    python3 convert_t5x_checkpoint_to_pytorch.py --t5x_retrieval_checkpoint_path=$HOME/t5_1_1_small --config_file=config.json\
...
def load_t5x_weights_in_t5(model, config, t5x_retrieval_checkpoint_path):
    """Replaces the params in model witht the T5X converted params."""
    variables = checkpoints.load_t5x_checkpoint(t5x_retrieval_checkpoint_path)
    converted = convert_t5x_to_pytorch(variables, num_layers=config.num_layers)
    state_dict = make_state_dict(converted)
    model.load_state_dict(state_dict, strict=True)


def convert_t5x_checkpoint_to_pytorch(t5x_retrieval_checkpoint_path, config_file, pytorch_dump_path):
    ...
    load_t5x_weights_in_t5(model, config, t5x_retrieval_checkpoint_path)
...
if __name__ == "__main__":
    ...
    # Required parameters
    parser.add_argument(
        "--t5x_retrieval_checkpoint_path", default=None, type=str, required=True, help="Path to the T5X checkpoint."
    )
    parser.add_argument(
        "--config_file",
        default=None,
    ...
    convert_t5x_checkpoint_to_pytorch(args.t5x_retrieval_checkpoint_path, args.config_file, args.pytorch_dump_path)
