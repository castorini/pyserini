...
for key in beir_keys:
    index_reader = IndexReader(f'indexes/faiss.beir-v1.0.0-{key}.contriever-ft.{date}.{commitid}')
    stats = index_reader.stats()
    md5 = compute_md5(f'indexes/faiss.beir-v1.0.0-{key}.contriever-ft.{date}.{commitid}.tar.gz')
    size = os.path.getsize(f'indexes/faiss.beir-v1.0.0-{key}.contriever-ft.{date}.{commitid}.tar.gz')
    print(f'    "beir-v1.0.0-{key}.contriever": {{')
    ...
    print(f'        "filename": "faiss.beir-v1.0.0-{key}.contriever-ft.{date}.{commitid}.tar.gz",')
    print(f'        "readme": "faiss.beir-v1.0.0-{key}.contriever-ft.{date}.{commitid}.README.md",')
    print(f'        "urls": [')
    print(f'            "https://rgw.cs.uwaterloo.ca/JIMMYLIN-bucket0/pyserini-indexes/faiss.beir-v1.0.0-{key}.contriever-ft.{date}.{commitid}.tar.gz"')
    print(f'        ],')
    print(f'        "md5": "{md5}",')
    print(f'        "size compressed (bytes)": {size},')
    print(f'        "documents": {stats["documents"]},')
