...
def get_aggregate_data_points(cache_dir):
    ...
    for task in [
        ...
        for qid in query_lens.keys():
            aggr_data[f"{task}_{qid}"] = (query_lens[qid], bm25qs_results[qid] - bm25_results[qid])
    return aggr_data
...
def scatter(xs, ys, log, output_path):
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="bm25vsbow_plots")
    parser.add_argument("--cache_dir", type=str, default="cache")
    args = parser.parse_args()
    aggr_data = get_aggregate_data_points(args.cache_dir)
    xs, ys = unpack_xy(aggr_data.values())
    os.makedirs(args.output_dir, exist_ok=True)
    scatter(xs, ys, True, os.path.join(args.output_dir, "scatter_log.pdf"))
    sum_counts = violin_log_bins(
