...
# Core analysis
# ---------------------------------------------------------------------------
def analyse(task: str, cache_dir: str, analysis_mode=AnalysisMode.DOCUMENT):
    """Yield stats for an iterable of plaintext documents (strings)."""
    ...
    try:
        if analysis_mode == AnalysisMode.DOCUMENT:
            key = "content"
            items = load_dataset("xlangai/bright", "documents", cache_dir=cache_dir)[
                task
            ]
            items_iter = tqdm(items, unit="docs", desc="Processing")
        else:
            key = "query"
            items = load_dataset("xlangai/bright", "examples", cache_dir=cache_dir)[
                task
            ]
            items_iter = tqdm(items, unit="docs", desc="Processing")
    except ImportError:
    ...
    if analysis_mode == AnalysisMode.DOCUMENT:
        vocab_size = len(df)
        idf_values = [
            math.log(1 + (N - df_t + 0.5) / (df_t + 0.5)) for df_t in df.values()
        ]
    ...
    collection_stats = {
        f"num_{item_name}": N,
        f"num_{item_name}_shorter_than_5_tokens": N_short,
        f"num_{item_name}_zero_tokens": N_zero,
    }
    if analysis_mode == AnalysisMode.DOCUMENT:
...
def main():
    ...
    parser.add_argument(
        ...
        help="Which part of the dataset to analyse (document or query).",
    )
    parser.add_argument
    ...
    for task in tasks:
        stats, lengths_sorted, idf_values_sorted, shorts = analyse(
            task, cache_dir, mode
        )
