...
#
# Note that this file can only be used after running `build_psg_doc_idmap.py` under the
# same folder, to prepare for the idmap.tsv files.
#
# Usage:
# python scripts/msmarco_v2/convert_psgrun_to_doc_run.py \
# 	--input runs/run.mspsg.dev.txt \
# 	--idmap-dir /path/to/idmap_dir  # generated by build_psg_doc_idmap.py \
...
def load_id_map_from_file(idmap_fn):
	psgid2docid = {}
	with open(idmap_fn) as f:
		for line in f:
...
def load_id_map_from_dir(idmap_dir):
	return {
		fn.replace(".idmap.tsv", ""): load_id_map_from_file(os.path.join(idmap_dir, fn))
		for fn in os.listdir(idmap_dir)
	}


def main(args):
	input_runfile, output_runfile = args.input, args.output
	idmap_dir = args.idmap_dir
	idmap = load_id_map_from_dir(idmap_dir)
...
if __name__ == "__main__":
	...
	parser.add_argument("--idmap-dir", type=str, required=True, help="dir to msmarco passage-doc id mapping .tsv files. Generated by `build_psg_doc_idmap.py`")
