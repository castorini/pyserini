...
def query_loader():
    queries = {}
    with open('../msmarco-passage_exp/queries.train.small.entity.json') as f:
        for line in f:
            ...
            queries[qid] = query
    with open('../msmarco-passage_exp/queries.dev.small.entity.json') as f:
        for line in f:
            query = json.loads(line)
            qid = query.pop('id')
            query['analyzed'] = query['analyzed'].split(" ")
            ...
            query['text_bert_tok'] = query['text_bert_tok'].split(" ")
            queries[qid] = query
    with open('../msmarco-passage_exp/queries.eval.small.entity.json') as f:
        for line in f:
            query = json.loads(line)
            qid = query.pop('id')
            query['analyzed'] = query['analyzed'].split(" ")
...
if __name__ == '__main__':
    ...
    print("add feature")
    fe = FeatureExtractor('../indexes/lucene-index-msmarco-passage-flex-ent-expanded/', max(multiprocessing.cpu_count()//2, 1))
