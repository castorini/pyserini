...
class TestLtrMsmarcoPassage(unittest.TestCase):
    def test_reranking(self):
        ...
        os.system(f'python -m pyserini.search.lucene.ltr   --input ltr_test/{inp} \
            --input-format tsv --model ltr_test/msmarco-passage-ltr-mrr-v1 --data passage \
            --index ~/.cache/pyserini/indexes/index-msmarco-passage-ltr-20210519-e25e33f.a5de642c268ac1ed5892c069bdc29ae3 \
            --ibm-model ltr_test/ibm_model/ --queries ltr_test --output-format tsv --output ltr_test/{outp}')

        result = subprocess.check_output(f'python tools/scripts/msmarco/msmarco_passage_eval.py \
            tools/topics-and-qrels/qrels.msmarco-passage.dev-subset.txt ltr_test/{outp}', shell=True).decode(sys.stdout.encoding)
