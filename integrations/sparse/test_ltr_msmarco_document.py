...
class TestLtrMsmarcoPassage(unittest.TestCase):
    def test_reranking(self):
        ...
        os.system('python scripts/ltr_msmarco/convert_queries.py --input tools/topics-and-qrels/topics.msmarco-doc.dev.txt --output collections/msmarco-ltr-document/queries.dev.small.json')
        ...
        result = subprocess.check_output(f'python tools/scripts/msmarco/msmarco_doc_eval.pytools/topics-and-qrels/qrels.msmarco-doc.dev.txt ltr_test/{outp_tsv}', shell=True).decode(sys.stdout.encoding)
        a,b = result.find('#####################\nMRR @10:'), result.find('\nQueriesRanked: 6980\n#####################\n')
        mrr = result[a+31:b]
