...
class TestSearchIntegration(unittest.TestCase):
    def setUp(self):
        ...
        #pre tokenized jsonl
        os.system(f'python pyserini/tokenize_json_collection.py --input {self.tmp}/cacm_jsonl/ --output {self.tmp}/cacm_bert_jsonl/ --tokenizer bert-base-uncased')

        self.pyserini_index_cmd = 'python -m pyserini.index'
        self.pyserini_search_cmd = 'python -m pyserini.search'

        self.cacm_jsonl_path = os.path.join(self.pyserini_root, f'{self.tmp}/cacm_jsonl')
        self.cacm_bert_jsonl_path = os.path.join(self.pyserini_root, f'{self.tmp}/cacm_bert_jsonl')

        self.cacm_index_path = os.path.join(self.pyserini_root, f'{self.tmp}/cacm_index')
        self.cacm_bert_index_path = os.path.join(self.pyserini_root, f'{self.tmp}/cacm_bert_index')
