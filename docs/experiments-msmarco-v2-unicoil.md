...
To build the indexes from scratch, we start from the [MS MARCO V2 passage corpus](https://github.com/castorini/anserini/blob/master/docs/experiments-msmarco-v2.md#passage-collection) that has already been processed with uniCOIL, i.e., gone through term reweighting.
...
To build the indexes from scratch, we start from the [MS MARCO V2 segmented document corpus](https://github.com/castorini/anserini/blob/master/docs/experiments-msmarco-v2.md#document-collection-segmented) that has already been processed with uniCOIL, i.e., gone through term reweighting.
Download the sparse representation of the corpus generated by uniCOIL:
