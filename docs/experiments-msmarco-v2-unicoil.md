...
```bash
wget https://rgw.cs.uwaterloo.ca/JIMMYLIN-bucket0/pyserini-indexes/lucene.unicoil-noexp.0shot.msmarco-v2-passage.tar.gz -P indexes/

# Alternate mirror
# wget https://vault.cs.uwaterloo.ca/s/bKbHmN6CjRtmoJq/download -O indexes/lucene.unicoil-noexp.0shot.msmarco-v2-passage.tar.gz

tar -xvf indexes/lucene.unicoil-noexp.0shot.msmarco-v2-passage.tar.gz -C indexes/
```
