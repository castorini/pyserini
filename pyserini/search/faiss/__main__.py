...
def init_query_encoder(
    ...
    if encoder:
        ...
        if _encoder_class == "uniir":
            ...
            if encoder_class is None:
                raise ValueError("UniIR's query encoder class is not available (as the uniir-for-pyserini package is not installed). Please run 'pip install pyserini[uniir]' to install the package.")
