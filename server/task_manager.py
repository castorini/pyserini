...
class TaskManager:
    """Core search functionality manager."""
    ...
    def get_index(self  ) -> Dict[str, IndexConfig]:
        """Get index configuration by name, or default if none specified."""
        return self.indexes

    def search(self, query: str, index_name: str, k: int = 10, qid: str = "", 
               ef_search: Optional[int] = None, encoder: Optional[str] = None, 
               query_generator: Optional[str] = None, shard: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        if not index_config.searcher or not not index_config.reader:
            ...
            index_config.reader = LuceneIndexReader.from_prebuilt_index(index_config.reader)
    ...
    def update_settings(self, index_name: str, ef_search: Optional[str] = None,
                       encoder: Optional[str] = None, query_generator: Optional[str] = None) -> None:
