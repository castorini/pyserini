...
if __name__ == "__main__":
    ...
    nq = load_dataset("Tevatron/wiki-ss-nq")['test']
    qid2answers = {}
    for example in nq:
    ...
    with open(args.run_file, "r") as f:
        for line in f:
            ...
            if len(parts) >= 3:
                # Handle TREC format: qid Q0 docid rank score system
                if len(parts) >= 6:
                    qid, _, docid, _, _, _ = parts[:6]
                else:
                    qid, docid = parts[0], parts[2] if len(parts) > 2 else parts[1]
                if qid not in results:
                    results[qid] = []
                results[qid].append(docid)
    

    ds = load_dataset("Tevatron/wiki-ss-corpus")['train']
    docid_to_idx = {str(docid): idx for idx, docid in enumerate(tqdm(ds["docid"]))}
